...
import {
  extractVendorBranchListItems,
  type VendorBranchListItem,
} from '@/features/utils/vendorBranches';
...
export default function VendorBranchDetails() {
  ...
  const { branchId, vendorId } = useParams();
  ...
  const branch = useMemo<VendorBranchListItem | undefined>(() => {
    const branches = vendorId
      ? extractVendorBranchListItems(vendorBranchesResponse?.data, vendorId)
      : extractVendorBranchListItems(allVendorsResponse?.data);
    ...
    return (
      <div className="md:py-10 space-y-6">
        ...
        <div className="border border-gray-200 rounded-lg p-4">
          <Text className="text-sm text-gray-500">Loading branch details…</Text>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="md:py-10 space-y-8">
      ...
      <div className="border border-gray-200 rounded-lg">
        <div className="bg-[#FAFAFA] p-3">
          <h2 className="text-gray-500 font-medium">Cards</h2>
        </div>
            ...
            <tbody>
              {branch.cards.map((card) => (
                ...
                >
                  ...
                  <td className="p-3 text-sm text-primary-900">
                    {formatCardPrice(card.card_price, card.currency)}
                  </td>
                  <td className="p-3 text-sm text-primary-900">
                  ...
                  <td className="p-3 text-sm">
                    <Tag
                      value={
                        card.card_status.charAt(0).toUpperCase() +
                        card.card_status.slice(1)
                      }
                      variant={
                        card.card_status === 'active' ? 'success' : 'warning'
                      }
                    />
                  </td>
                </tr>
          ...
          {branch.cards.length === 0 && (
            <div className="p-4">
              <Text className="text-sm text-gray-500">
                No cards found for this branch.
              </Text>
            </div>
          )}
        </div>
      ...
      <div className="border border-gray-200 rounded-lg">
        ...
        <div className="overflow-x-auto">
          ...
          {payments.length === 0 && (
            <div className="p-4">
              <Text className="text-sm text-gray-500">
                No payments found for this branch.
              </Text>
            </div>
          )}
        </div>
