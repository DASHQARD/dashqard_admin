import type { SubmitHandler } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { Button, Combobox, Modal } from '@/components';
import { Controller } from 'react-hook-form';
import { usePersistedModalState } from '@/hooks';
import { useCustomForm } from '@/libs';
import { MODALS } from '@/utils/constants';
import { ticketsManagementMutations } from '@/features/hooks/ticketsManagement';
import { z } from 'zod';
import { useEffect } from 'react';

const updateTicketStatusSchema = z.object({
  status: z.string().min(1, 'Status is required'),
});

type UpdateTicketStatusSchemaType = z.infer<typeof updateTicketStatusSchema>;

type TicketData = {
  id: number | string;
  name?: string;
  email?: string;
  subject?: string;
  status?: string;
};

export function UpdateTicketStatus() {
  const modal = usePersistedModalState<TicketData>({
    paramName: MODALS.TICKETS_MANAGEMENT.PARAM_NAME,
  });

  const { useUpdateTicketStatus } = ticketsManagementMutations();
  const updateTicketStatusMutation = useUpdateTicketStatus();

  const form = useCustomForm({
    resolver: zodResolver(updateTicketStatusSchema),
    defaultValues: {
      status: '',
    },
  });

  useEffect(() => {
    if (modal.modalData) {
      form.reset({
        status: modal.modalData.status || '',
      });
    }
  }, [modal.modalData, form]);

  const onSubmit: SubmitHandler<UpdateTicketStatusSchemaType> = (data) => {
    if (!modal.modalData?.id) return;

    updateTicketStatusMutation.mutate(
      {
        id: String(modal.modalData.id),
        data: { status: data.status },
      },
      {
        onSuccess: () => {
          modal.closeModal();
          form.reset();
        },
      }
    );
  };

  const statusOptions = [
    { label: 'Open', value: 'open' },
    { label: 'In Progress', value: 'in_progress' },
    { label: 'Resolved', value: 'resolved' },
    { label: 'Closed', value: 'closed' },
  ];

  return (
    <Modal
      panelClass="!w-[680px]"
      title="Update Ticket Status"
      isOpen={modal.isModalOpen(
        MODALS.TICKETS_MANAGEMENT.CHILDREN.UPDATE_STATUS
      )}
      setIsOpen={(isOpen) => {
        if (!isOpen) {
          modal.closeModal();
          form.reset();
        }
      }}
      position="side"
    >
      <form
        onSubmit={form.handleSubmit(onSubmit)}
        className="flex flex-col h-full"
      >
        <div className="h-full px-6 flex flex-col gap-6 justify-between">
          <div className="grow overflow-y-auto py-6">
            <div className="space-y-6">
              <Controller
                control={form.control}
                name="status"
                render={({ field }) => (
                  <Combobox
                    label="Status"
                    placeholder="Select status"
                    options={statusOptions}
                    value={field.value}
                    onChange={(e: { target: { value: string } }) => {
                      field.onChange(e.target.value);
                    }}
                    error={form.formState.errors.status?.message}
                  />
                )}
              />
            </div>
          </div>

          <div className="flex gap-4 justify-end pt-4 border-t border-gray-200">
            <Button
              type="button"
              variant="outline"
              onClick={() => {
                modal.closeModal();
                form.reset();
              }}
            >
              Cancel
            </Button>
            <Button
              type="submit"
              variant="secondary"
              disabled={updateTicketStatusMutation.isPending}
            >
              {updateTicketStatusMutation.isPending
                ? 'Updating...'
                : 'Update Status'}
            </Button>
          </div>
        </div>
      </form>
    </Modal>
  );
}
